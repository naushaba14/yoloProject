...
class Canvas(QWidget):
        ...
        if self.drawing():
            ...
            if self.current:
                ...
                if self.out_of_pixmap(pos):
                    ...
                    # Attract line to starting point and colorize to alert the user:
                    pos = self.current[0]
                ...
                # Update self.prev_point only when self.current is not None:
                self.prev_point = pos
                self.current.highlight_clear()
            else:
                # Handle the case when self.current is None appropriately
                pass
            self.repaint()
            return
    ...
    def paintEvent(self, event):
        ...
        if self.current is not None and len(self.line) == 2:
            ...
            rect_width = abs(right_bottom.x() - left_top.x())  # Ensure positive width
            rect_height = abs(right_bottom.y() - left_top.y())  # Ensure positive height
            p.setPen(self.drawing_rect_color)
            brush = QBrush(Qt.BDiagPattern)
            p.setBrush(brush)
            # Ensure coordinates are integers by rounding
            left = round(min(left_top.x(), right_bottom.x()))
            top = round(min(left_top.y(), right_bottom.y()))
            # Ensure width and height are integers by rounding
            rect_width = round(rect_width)
            rect_height = round(rect_height)
            p.drawRect(left, top, rect_width, rect_height)

        if self.drawing() and not self.prev_point.isNull() and not self.out_of_pixmap(self.prev_point):
            p.setPen(QColor(0, 0, 0))
            # Round coordinates to ensure they are integers
            x = round(self.prev_point.x())
            p.drawLine(x, 0, x, self.pixmap.height())
            y = round(self.prev_point.y())
